...
print(aic_bic_x1)
#viterbi
m=as.numeric(aic_bic_x1[1,len_table])
initP_last=rep(1/m,m)
transP_last=matrix(0.3/m,m,m)+matrix(c(rep(0.7,m),rep(0,m*(m-1))),m,m)
...
p_t=initP_est_x1
s[1]=which.max(p_t)
xr[1]=rpois(1,lambda_est_x1[s[1]])
for (t in 2:T){
  p_t=initP_est_x1%*%transP_est_x1
  s[t]=which.max(p_t)
  xr[t]=rpois(1,lambda_est_x1[s[t]])
}
par(mar = c(2, 2, 2, 7), xpd = TRUE)
plot(density(x1))
par(new=TRUE)
